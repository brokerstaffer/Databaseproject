-- 0094: restore the unconditional guard in the A5 scoped branch.
-- 0093 made the guard conditional on the WHERE text, but the scoped branch's grouped
-- agent_mls_stats join is in the SELECT, not the WHERE, so the guard stopped firing there
-- and MLS-only searches regressed from 693 ms to 3,897 ms. Caught in regression testing.
