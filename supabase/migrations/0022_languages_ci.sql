-- 0022_languages_ci.sql
-- Languages filter compares case-insensitively (user types 'spanish', data stores 'Spanish').
-- Function body otherwise identical to 0020.
...
begin
  ...
  if f is not null then
    if jsonb_array_length(coalesce(f->'languages', '[]'::jsonb)) > 0 then
      -- case-insensitive: 'spanish' matches stored 'Spanish'
