-- 0011_filter_search_v3.sql
-- Two fixes on top of 0006:
--   (1) Name include/exclude now match by CONTAINS (ilike), not exact full_name.
--   (2) Office mode reads the STORED offices.agent_count instead of recomputing it live
--       on every query (the live sub-count was the source of the "Agents" lag).
-- One-time backfill ensures the stored counts are correct as of now; the importer keeps
-- them current on every ingest.

-- ---- one-time backfill of the stored office agent counts ----
update offices o
set agent_count = (select count(*) from agents a where a.office_id = o.id);
...
begin
  ...
  if p_mode = 'office' then
    ...
    -- agent_count comes from the stored offices.agent_count column (via to_jsonb(o)); only the
    -- top-agent name preview is computed live.
    execute format($q$
  ...
  -- NAME: include/exclude match by CONTAINS (ilike), so a selected/typed value that is
  -- part of a name still matches. Free-text "nameQuery" (top search bar) also CONTAINS.
  f := p_filters->'name';
  if f is not null then
    if jsonb_array_length(coalesce(f->'include', '[]'::jsonb)) > 0 then
      kconds := '{}';
