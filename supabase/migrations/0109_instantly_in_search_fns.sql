-- 0109: the search functions read BOTH sequencers.
--
-- 0108 gave Instantly the same table shape as EmailBison and added the views. This points the five
-- search functions at those views, so Bounced, Campaigns, Client campaign, In campaign and
-- In multiple campaigns stop meaning "EmailBison only", and adds the two reply-provenance columns.
--
-- What changes for the operator, measured before applying:
--     bounced agents        2,342 ->  ~3,712
--     agents in a campaign 59,731 -> ~117,971   (58,240 were in Instantly and no Bison campaign)
--     replied agents        6,988 -> unchanged  (0108 already unioned both providers)
--
-- Generated by transforming the LIVE pg_get_functiondef output rather than hand-editing the
-- previous migration, with every substitution count-asserted. Re-emitting a stale copy of
-- fn_agent_where is what broke agent search once already.
--
-- NO NEW PARAMETERS. Adding a defaulted parameter to any of these creates a SECOND overload
-- instead of replacing the function, which makes every existing call ambiguous -- that failure
-- took agent search down twice in one day. The signatures below are byte-identical to the live
-- ones.

-- v_agent_campaigns gains the two flags so fn_agent_page_ids' single aggregate can compute
-- bool_or(bounced) from it. CREATE OR REPLACE VIEW may append columns, not reorder them, so these
-- go on the end.
...
REVOKE ALL ON public.v_agent_campaigns FROM public, anon, authenticated;
...
begin
  ...
  if v_client_ids is not null then
    if coalesce(p_filters->>'orchClientMode', 'include') = 'exclude' then
      parts := parts || format('id not in (select b.agent_id from v_agent_campaigns b where b.client_id = any(%1$L::uuid[]) and b.agent_id is not null union all select l.agent_id from orch_client_leads l where l.client_id = any(%1$L::uuid[]) and l.agent_id is not null and not exists (select 1 from v_agent_campaigns x where x.client_id = l.client_id))', v_client_ids);
    else
      parts := parts || format('id in (select b.agent_id from v_agent_campaigns b where b.client_id = any(%1$L::uuid[]) and b.agent_id is not null union all select l.agent_id from orch_client_leads l where l.client_id = any(%1$L::uuid[]) and l.agent_id is not null and not exists (select 1 from v_agent_campaigns x where x.client_id = l.client_id))', v_client_ids);
    end if;
  end if;
  ...
  -- column to lean on (mls_count is one), so this groups v_agent_campaigns -- both providers,
  ...
  if coalesce(p_filters->>'multiCampaign', '') = 'true' then
    parts := parts || 'id in (select agent_id from v_agent_campaigns where agent_id is not null group by agent_id having count(distinct campaign_id) >= 2)'::text;
  end if;
  ...
  if f is not null then
    ...
    if f->>'bounced' = 'has' then
      parts := parts || 'id in (select agent_id from v_bounced_agents)'::text;
    elsif f->>'bounced' = 'missing' then
      parts := parts || 'id not in (select agent_id from v_bounced_agents)'::text;
    end if;
    -- C5: already sitting in a client's Bison campaign
    if f->>'inCampaign' = 'has' then
      parts := parts || 'id in (select agent_id from v_campaign_agents)'::text;
    elsif f->>'inCampaign' = 'missing' then
      parts := parts || 'id not in (select agent_id from v_campaign_agents)'::text;
    end if;
  end if;
...
AS $function$
  ...
  || case when p_sort_by = 'has_bounced' then
       ...
       format('(a.id in (select agent_id from v_bounced_agents)) %s, sales_volume desc nulls last',
...
begin
  ...
  -- column. They are answered by ONE aggregate over v_agent_campaigns -- both providers, 110k
  -- Bison rows plus ~168k Instantly rows -- joined once and sorted as a narrow (id, key) set
  -- like everything else. Reply source and Reply campaign join the same narrow path.
  if p_sort_by in ('has_replied', 'has_bounced', 'client_campaigns', 'campaign_count',
                   'reply_providers', 'reply_campaigns') then
    v_join := ' left join (select b.agent_id,'
           ...
           || ' from v_agent_campaigns b left join orch_clients c on c.id = b.client_id'
           ...
           || ' left join v_agent_reply_sources rs on rs.agent_id = a.id';
    ...
    v_sk := case p_sort_by
              ...
              when 'reply_providers' then 'rs.reply_providers'
              when 'reply_campaigns' then 'rs.reply_campaigns'
              else 'k.cc' end;
  else
...
begin
  ...
  if v_client_ids is not null then
    if coalesce(p_filters->>'orchClientMode', 'include') = 'exclude' then
      parts := parts || format('id not in (select a.office_id from agents a join (select b.agent_id from v_agent_campaigns b where b.client_id = any(%1$L::uuid[]) and b.agent_id is not null union all select l.agent_id from orch_client_leads l where l.client_id = any(%1$L::uuid[]) and l.agent_id is not null and not exists (select 1 from v_agent_campaigns x where x.client_id = l.client_id)) s on s.agent_id = a.id where a.office_id is not null)', v_client_ids);
    else
