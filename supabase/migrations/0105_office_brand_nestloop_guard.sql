-- 0105: the Office and Brand branches get the nested-loop guard too.
--
-- 0093/0094 added it after every fn_agent_where assignment, but the office-grain branches build
-- their WHERE from fn_office_where and were missed. A saved view that carries an MLS plus a
-- range filter resolves at agent grain inside the office WHERE, so the same grouped
-- agent_mls_stats subquery appears and the planner mis-estimates it the same way.
--
-- Measured on the BHS Base Camp saved view:
--     office view   4,094 ms -> (count alone) 2,140 ms with nested loops, 191 ms without
--     brand view    2,041 ms
--
-- Every other Office/Brand filter was already fast (5-360 ms) and is unaffected: the guard only
-- fires when the generated WHERE actually contains agent_mls_stats.
