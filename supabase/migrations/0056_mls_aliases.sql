-- 0056: editable MLS abbreviations. mls.aliases keeps every former code so the
-- scraper's rows (matched by code) still resolve to the same MLS after a rename;
-- the MLS search also matches old abbreviations.
alter table mls add column if not exists aliases text[] not null default '{}';
