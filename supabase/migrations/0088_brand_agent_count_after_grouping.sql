-- 0088: Brand view's "Agent Count" filters the BRAND's agents, not each office's.
--
-- Reported: "BRAND VIEW: Agent count is filtering for agents per office and not for agents in
-- brand." Confirmed. fn_office_where turns the agentCount filter into "agent_count between ..."
-- against the offices table, and the brand branch of fn_filter_search puts that in the WHERE
-- that runs BEFORE "group by brand". So it selected OFFICES of that size, then grouped and
-- summed whatever survived -- never asking how many agents the brand has.
--
-- Two distinct wrong answers, both measured at Agent Count min = 100:
--
--   Wrong brands. 471 brands actually have >= 100 agents; the view showed 93. The 378 missing
--   ones are brands made of many small offices:
--       NextHome            415 offices,  2,588 agents,  biggest single office 62
--       Engel & Volkers     176 offices,  2,055 agents,  biggest single office 53
--       Side                238 offices,  1,090 agents,  biggest single office 57
--       Houlihan Lawrence    29 offices,  1,041 agents,  biggest single office 96
--
--   Wrong number. The Agent Count column summed only the offices that passed the filter:
--       Keller Williams   showed 64,302   true brand total 84,509
--       Coldwell Banker   showed 19,170   true brand total 48,307
--       RE/MAX            showed  2,239   true brand total 30,385
--
-- Fix: drop agentCount from the pre-aggregation WHERE and apply it to the grouped result,
-- where "agent_count" is already coalesce(sum(agent_count), 0) -- the brand total the column
-- displays. Wrapping the grouped set as g0 and selecting from it into g means the one
-- condition covers all three consumers: the row page, the brand count, and salesVolumeTotal.
--
-- fn_range_cond is reused as-is, so the bucket options (1-5, 5-10, 10-20, 20+) and the custom
-- min/max behave identically -- they simply now measure the brand instead of an office.
--
-- Scope is deliberately just Agent Count, as requested. Sales Volume and Closed Units remain
-- office-level filters in this view ("brands having an office that does $100M+"), which is a
-- separate question and was left alone.
--
-- Office view is untouched: it still calls fn_office_where(p_filters) with agentCount included,
-- which is the correct meaning there.
