-- 0042_city_filter_perf.sql
-- PRODUCTION FIX: multi-value city/county filters (e.g. the 40-city 'Douglas Realty' saved
-- view) generated a per-value OR chain that re-evaluated fn_city_match_key(col) once per
-- VALUE per row (~93M regex calls at 40 values), exceeding the API's 8s statement timeout —
-- the UI rendered the 500 as "0 agents", and saved-view include/exclude (which resolve those
-- same filters) failed the same way. Values are now grouped by state part and matched as
-- fn_city_match_key(col) = ANY(keys) — one key evaluation per row, functional-index friendly.
-- Semantics identical. Idempotent.
    ...
    foreach side in array array['values', 'excludeValues'] loop
      if jsonb_typeof(f->side) = 'array' and jsonb_array_length(f->side) > 0 then
        ...
        if field = 'city' or field = 'county' then
          ...
          if array_length(arr, 1) > 0 then
            if field = 'city' then
              vconds := vconds || format('fn_city_match_key(office_city) = ANY(%L::text[])', arr);
            else
          ...
          for vst in select distinct upper((regexp_match(x.v, ',\s*([A-Za-z]{2})\s*$'))[1])
            ...
            if array_length(arr, 1) > 0 then
              if field = 'city' then
                vconds := vconds || format('(fn_city_match_key(office_city) = ANY(%L::text[]) and coalesce(upper(office_state), fn_city_embedded_state(office_city)) = %L)', arr, vst);
              else
