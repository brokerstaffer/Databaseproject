-- 0038_bison_client_leads.sql
-- D1 / Phase 3: EmailBison is the source of truth for a client's leads once their campaigns
-- hold leads. bison_client_leads mirrors campaign membership (synced by /api/cron/bison-sync):
-- one row per (client, campaign, email), agent_id matched by email where the lead exists in
-- our DB (null = in Bison but not in the database). The client filter shows, per client:
--   synced client   -> exactly the Bison-matched agents (the ChuckTown 1,004)
--   unsynced client -> the scraper/orchestrator lead list (new clients pre-send)
-- Idempotent.

create table if not exists bison_client_leads (
  id uuid primary key default gen_random_uuid(),
  client_id uuid not null references orch_clients(id) on delete cascade,
  ...
  unique (client_id, campaign_id, email)
);
create index if not exists idx_bcl_client on bison_client_leads (client_id);
create index if not exists idx_bcl_agent on bison_client_leads (agent_id);
revoke all on bison_client_leads from public, anon, authenticated;

-- client filter v4: per client, Bison-synced set when present, else the orchestrator list
