-- 0098: the two filters that dominated the slow tail of a 325-combination sweep.
--
-- Every filter and every PAIR of filters was timed against the live database: 325 tests, zero
-- failures, 206 under 250 ms and 272 under 1 s. But 14 ran over 3 s, and nearly all of them were
-- pairs containing one of just two filters, each of which was an unindexable formulation of
-- data that IS indexed:
--
--     title exclude   3,566 ms alone   (worst pair: with saved-view exclude, 10,934 ms)
--     languages       3,001 ms alone
--
-- TITLE EXCLUDE emitted "(title is null or not (fn_title_tokens(title) && ...))". A NOT over a
-- GIN index cannot use that index, so it scanned all 1.13M agents evaluating fn_title_tokens per
-- row. Rewritten as an anti-join over the positive form, which uses idx_agents_title_tokens.
-- Semantics identical: the old form kept a row when the title was NULL or nothing overlapped,
-- and NOT EXISTS keeps a row when the positive is not true -- the same two cases.
--
-- LANGUAGES emitted "exists (select 1 from unnest(languages) lang where lower(lang) = any(...))".
-- The unnest defeats idx_agents_languages completely:
--     unnest + lower()          3,001 ms
--     languages && '{Spanish}'      0.98 ms
-- Case-insensitivity is preserved rather than quietly dropped: fn_lower_arr lowercases the
-- stored array and gets its own GIN index, so a value stored as "spanish" still matches a search
-- for "Spanish". All 20 stored values are initcap today, but the enricher could write either,
-- and changing the matching rule would be a behaviour change rather than an optimisation.
--
-- fn_lower_arr exists because Postgres rejects "array(select ...)" as an index expression -- it
-- is a subquery. Same reason fn_title_tokens exists. IMMUTABLE and PARALLEL SAFE so it can back
-- an index and not block parallel scans.
--
-- The GIN index on fn_lower_arr(languages) is created separately with CONCURRENTLY, which cannot
-- run inside the transaction this file is applied in.
--
-- NOTE ON THE PREVIOUS ATTEMPT: an earlier version of this file was generated from a stale
-- scratchpad dump and contained the PRE-0091 two-argument fn_agent_where. Applying it recreated
-- the two-argument overload -- making every 2-arg call ambiguous and breaking agent search -- and
-- would have reverted 0091's location anti-join. It was detected and dropped, and this file is
-- regenerated from the live definition with assertions that the alias parameter and the location
-- anti-join are both still present.

CREATE OR REPLACE FUNCTION public.fn_lower_arr(p text[])
 RETURNS text[]
 LANGUAGE sql
 IMMUTABLE
 PARALLEL SAFE
AS $function$
  select case when p is null then null else array(select lower(x) from unnest(p) x) end;
$function$;
