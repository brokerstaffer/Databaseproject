-- 0100: Bounced becomes sortable, the computed sorts get the fast path, and campaigns become
-- visible and filterable.
--
-- 1. BOUNCED SORTABLE. Replied was already sortable; Bounced simply had no case in
--    fn_agent_order. Added, mirroring Replied exactly.
--
-- 2. THE COMPUTED SORTS WERE THE LAST SLOW ONES IN THE APP. Sorting by Replied cost 2,848 ms and
--    by Client campaign 2,776 ms, against 117 ms for an ordinary column. They sort by a computed
--    flag rather than a real column, so 0086 deliberately excluded them from the narrow-id path
--    and they dragged all 1.13M full-width rows through the sort. Adding Bounced as-is would
--    have created a third one.
--
--    fn_agent_page_ids now answers all four -- Replied, Bounced, Client campaign and the new
--    Campaigns count -- from ONE aggregate over bison_client_leads (110k rows behind
--    idx_bcl_agent), joined once and sorted as a narrow (id, key) set like every other column.
--    Agents in no campaign coalesce to false/0 so they sort where the table renders them.
--
-- 3. CAMPAIGN COUNT. bison_client_leads already stores campaign_id and campaign_name, so nothing
--    new is collected. Per page of 20 the count costs 0.198 ms. Added to all three row-shaping
--    blocks and to the MLS-scoped payload, so the column is populated whichever path serves the
--    page.
--
-- 4. MULTI-CAMPAIGN FILTER. "In 2+ campaigns", mirroring multiMls. There is no maintained
--    counter to lean on (mls_count is one), so it groups bison_client_leads: 203 ms. Campaigns
--    of the same client count separately -- the question is how many campaigns the agent is in.
--
--    Current data: 58,184 agents are in a campaign, 28,250 of them (48.6%) in two or more,
--    6,406 in three or more, and one agent is in eight.
--
-- Nothing is removed. The superseded client_campaigns branch and the wide-row fallback are both
-- left in place, just no longer reached.
...
begin
  ...
  if p_sort_by in ('has_replied', 'has_bounced', 'client_campaigns', 'campaign_count') then
    ...
    v_sk := case p_sort_by
              when 'has_replied'    then 'coalesce(k.hr, false)'
