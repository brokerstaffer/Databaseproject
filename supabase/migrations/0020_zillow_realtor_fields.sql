-- 0020_zillow_realtor_fields.sql
-- Zillow/Realtor "keep separate" fields (spec CSV: merged=No) as dedicated agent columns —
-- these are ALL-TIME stats and extras Courted doesn't have, so they never mix with the LTM
-- metrics. Plus one grouped filter for them: p_filters->'zillowRealtor'
--   { languages: string[], totalSales/avgPriceAllTime/avgVolumeAllTime: {min,max}, hasLinkedin: bool }

alter table agents
  add column if not exists linkedin_url text,
  add column if not exists languages text[],
  add column if not exists total_sales_all_time numeric,
  add column if not exists avg_price_all_time numeric,
  add column if not exists avg_sales_volume_all_time numeric,
...
create index if not exists idx_agents_total_sales_at on agents (total_sales_all_time);
create index if not exists idx_agents_avg_price_at on agents (avg_price_all_time);
create index if not exists idx_agents_avg_vol_at on agents (avg_sales_volume_all_time);
create index if not exists idx_agents_linkedin on agents (linkedin_url) where linkedin_url is not null;
...
begin
  ...
  -- ZILLOW/REALTOR extras (grouped filter)
  f := p_filters->'zillowRealtor';
  if f is not null then
    if jsonb_array_length(coalesce(f->'languages', '[]'::jsonb)) > 0 then
      parts := parts || format('languages && %L::text[]', array(select jsonb_array_elements_text(f->'languages')));
    end if;
