-- 0012_namequery_highlight.sql
-- Change the top-bar name search (p_filters->>'nameQuery') from a WHERE filter into a sort
-- PRIORITY: matching agents float to the TOP of the current filtered list instead of removing
-- the non-matching rows. The screen highlights the matches (light green) client-side. The Name
-- include/exclude filter remains the real "narrow down" tool.
