-- 0069 (C1): bounced indicator maintained by the sync's bounce-folder sweep.
alter table bison_client_leads add column if not exists bounced boolean not null default false;
...
begin
  if p_mode = 'mls' then
    ...
    v_sort_col := case p_sort_by when 'mls' then 'label' when 'agents' then 'agents' when 'offices' then 'offices' when 'units' then 'units' else 'sales_volume' end;
