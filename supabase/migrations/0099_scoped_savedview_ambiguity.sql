-- 0099: a saved view combined with an MLS filter no longer errors.
--
-- Found by timing all 400 saved-view combinations. 395 passed; 5 failed outright with
--   ERROR: column reference "sales_volume" / "closed_transactions" is ambiguous
-- namely Douglas Elliman Las Vegas, Douglas Elliman NYC, Keyes, Properties & Estates FL and
-- Simply Vegas -- each combined with an MLS filter.
--
-- The A5 scoped branch joins agents to a per-agent aggregate of agent_mls_stats that exposes
-- the SAME metric names (sales_volume, units, closed_transactions, approx_gci, ...). Saved
-- views inline their stored conditions with BARE column names (0081), so any view carrying a
-- metric range filter produced a reference matching both relations and Postgres rejected the
-- statement. Not slowness -- a hard failure, which the UI used to render as "0 agents".
--
-- Fix: apply the WHERE to agents before joining sc. The subquery keeps the alias 'a' so every
-- a.* reference, to_jsonb(a) and the five correlated subqueries are untouched; it is used once
-- and has nothing volatile, so it still inlines and the LIMIT still pushes down.
