-- 0053 (A5 phase 3b): MLS-scoped production filtering, display and sort.
-- Gate: every selected MLS must have >= 90% per-MLS stats coverage (mls.stats_agents /
-- member_agents, maintained by fn_refresh_mls_freshness). Until the 15-day refreshes
-- fill agent_mls_stats, all behavior is byte-identical to today. When the gate opens:
-- production range filters evaluate against the SUM across the selected MLSs (HAVING
-- over one hashed group-by — never per-row), the grid shows and sorts by the scoped
-- numbers, and rows carry mls_scoped=true.

CREATE OR REPLACE FUNCTION public.fn_bucket_cond_expr(p_col text, p_label text)
 RETURNS text
 LANGUAGE sql
 IMMUTABLE
AS $function$
  select case p_label
    when '$0-5M'    then format('%1$s >= 0 and %1$s < 5000000', p_col)
    when '$5-10M'   then format('%1$s >= 5000000 and %1$s < 10000000', p_col)
    when '$10-20M'  then format('%1$s >= 10000000 and %1$s < 20000000', p_col)
    when '$20-50M'  then format('%1$s >= 20000000 and %1$s < 50000000', p_col)
    when '$50-100M' then format('%1$s >= 50000000 and %1$s < 100000000', p_col)
    when '$100M+'   then format('%1$s >= 100000000', p_col)
    when '1-5'   then format('%1$s >= 1 and %1$s <= 5', p_col)
    when '5-10'  then format('%1$s > 5 and %1$s <= 10', p_col)
    when '10-20' then format('%1$s > 10 and %1$s <= 20', p_col)
    when '20+'   then format('%1$s > 20', p_col)
    when '0-1yr'   then format('%1$s >= 0 and %1$s < 12', p_col)
    when '1-3yrs'  then format('%1$s >= 12 and %1$s < 36', p_col)
    when '3-5yrs'  then format('%1$s >= 36 and %1$s < 60', p_col)
    when '5-10yrs' then format('%1$s >= 60 and %1$s < 120', p_col)
    when '10+yrs'  then format('%1$s >= 120', p_col)
    when '$0-100K'    then format('%1$s >= 0 and %1$s < 100000', p_col)
    when '$100-250K'  then format('%1$s >= 100000 and %1$s < 250000', p_col)
    when '$250-500K'  then format('%1$s >= 250000 and %1$s < 500000', p_col)
    when '$500K-1M'   then format('%1$s >= 500000 and %1$s < 1000000', p_col)
    when '$1M+'       then format('%1$s >= 1000000', p_col)
    else 'true'
  end;
$function$;

CREATE OR REPLACE FUNCTION public.fn_range_cond_expr(p_col text, f jsonb)
 RETURNS text
 LANGUAGE plpgsql
 IMMUTABLE
AS $function$
...
begin
  ...
  if jsonb_array_length(coalesce(f->'buckets', '[]'::jsonb)) > 0 then
    for b in select jsonb_array_elements_text(f->'buckets') loop
      bconds := bconds || fn_bucket_cond_expr(p_col, b);
    end loop;
  ...
  if vmin is not null then conds := conds || format('%s >= %s', p_col, vmin); end if;
  if vmax is not null then conds := conds || format('%s <= %s', p_col, vmax); end if;
        ...
        select to_jsonb(a)
            ...
            || jsonb_build_object('mls_scoped', true,
                 ...
                 'source_stats', (select jsonb_agg((to_jsonb(s) - 'agent_id') order by s.source)
                                    from agent_source_stats s where s.agent_id = a.id)) as t
...
grant execute on function fn_bucket_cond_expr(text, text) to anon, authenticated;
grant execute on function fn_range_cond_expr(text, jsonb) to anon, authenticated;
