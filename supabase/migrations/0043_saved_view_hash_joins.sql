-- 0043_saved_view_hash_joins.sql
-- Companion to 0042: saved-view resolution moves from per-row correlated EXISTS to hashed
-- IN / NOT IN — each referenced view's subquery is evaluated ONCE and probed per row.
-- agents.id / offices.id are NOT NULL PKs, so NOT IN cannot be null-poisoned; fn_filter_search
-- and fn_filter_ids get function-level work_mem so the ~770k-id hash stays in memory instead
-- of degrading to a materialized O(n^2) subplan (the original timeout mechanism).
-- Also raises the PostgREST statement ceiling to 30s: fn_filter_search legitimately runs its
-- WHERE twice (count + page), and 8s left no headroom for heavy-but-valid searches.
