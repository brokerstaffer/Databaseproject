-- 0091: excluding locations stops reading the whole table (second attempt; 0089 was reverted).
--
-- Reported as "filtering by different locations keeps loading and never finishes". Excluding
-- places emitted "not coalesce((<or of column tests>), false)" -- unindexable, so Postgres read
-- all 1.13M agents and deformed each wide row to reach up to nine location columns. The 20-row
-- page was 0.46 ms; the count(*) was 6,379 ms. Including the same places costs 260 ms, because
-- that direction uses the city-key and state indexes that already exist.
--
-- So: ask it the other way round. Find the agents that DO match, keep everyone else.
--
-- WHY NOT "id not in (select ...)" -- that was 0089, and it was reverted the same day. It is
-- alias-free and looks equivalent, but NOT IN cannot always be planned as an anti-join because
-- of its NULL rules, and when the estimate exceeds work_mem it silently degrades to a per-row
-- subplan. City excludes improved 6,132 -> 1,061 ms, but excluding two states went from
-- 1,797 ms to over ten minutes. NOT EXISTS is planned as a real anti-join (verified:
-- "Parallel Hash Anti Join") and has no such cliff.
--
-- Old vs new, both counted in the same statement so the scraper's writes cannot skew it. All
-- six agree to the row:
--
--     case                 old ms    new ms   change   rows
--     city x7 (3 kinds)      5,985       638     -89%   1,088,826
--     city x1 office         1,309       421     -68%   1,115,649
--     zip x2                 1,303       260     -80%   1,129,895
--     county                 1,380       213     -85%   1,134,351
--     state CA                 605       588      -3%     950,205
--     state CA + TX          1,040     1,495     +44%     810,175
--
-- Multi-state exclude is the one case that gets slower. Accepted deliberately: it is 455 ms on
-- a filter that was never the complaint, and the worst case across all excludes drops from
-- 6.0 s to 1.5 s.
--
-- THE ALIAS. NOT EXISTS has to name the outer row, and fn_agent_where's output is pasted into
-- relations with different aliases: most callers scan "from agents a", but the saved-view blocks
-- nest a second agents scan aliased a9 (here and in fn_office_where). Assuming 'a' would produce
-- "axl.id = a.id" inside a scope where only a9 exists -- a hard error, or worse, a silent
-- self-comparison that matches every row. So the alias is now an explicit parameter, defaulted
-- to 'a' (so the five callers that already use it need no change) and passed as 'a9' at the two
-- nesting sites. The saved-view emissions that were hard-coding "a." now use it too, and the
-- INCLUDE branch -- which inlines into the OUTER relation rather than a nested scan -- passes
-- the caller's own alias down. quote_ident keeps it out of injection range.

CREATE OR REPLACE FUNCTION public.fn_agent_where(p_source text, p_filters jsonb, p_alias text DEFAULT 'a')
 RETURNS text
 LANGUAGE plpgsql
 STABLE
...
-- fn_office_where: its nested agents scan is aliased a9, so the inner predicate must correlate to a9.
          ...
          begin
            ...
            if vv is not null then
              ...
              if vmode = 'office' then
                ...
                kconds := kconds || format('id in (select a9.office_id from agents a9 where a9.office_id is not null and (%s))', fn_agent_where(vsrc, vv, 'a9'));
              end if;
            end if;
          exception when others then
