-- 0074: the MLS view's "Data refreshed" column becomes sortable (A20).
--
-- Every other column in the MLS grain (MLS name, Agents, Offices, Sales volume, Units) was
-- already sortable; "Data refreshed" was the only header without a sort control, because
-- fn_filter_search's MLS branch whitelists the sortable columns and 'updated' was missing
-- from that list. Anything not on the whitelist silently falls back to sales_volume, so the
-- column could not be sorted even if the UI offered it.
--
-- 'updated' is already a column of the branch's g CTE:
--     to_char(m.bulk_refreshed_at, 'YYYY-MM-DD') as updated
-- It is text, but in ISO YYYY-MM-DD form, so lexical ordering IS chronological ordering. The
-- branch's ORDER BY already appends "nulls last", so MLSs that have never reported a bulk
-- refresh sort to the bottom in both directions rather than crowding the top.
--
-- This is a CREATE OR REPLACE of the whole function because that is the only way Postgres
-- lets you edit one. The body is the live definition with a single line changed — the
-- v_sort_col whitelist in the p_mode = 'mls' branch gains "when 'updated' then 'updated'".
-- Nothing else in the function differs (verified by diffing against pg_get_functiondef).
