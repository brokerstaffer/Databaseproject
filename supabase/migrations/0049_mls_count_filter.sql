-- 0049 (B3): filter by number of MLS affiliations. agents.mls_count is a
-- trigger-maintained denormalized count over agent_mls (live EXISTS/HAVING was
-- measured at 4-6s; the indexed column is 0.7-1.3s and stays combinable with heavy
-- filters under the 30s API ceiling). Backfill runs separately in batches.
alter table agents add column if not exists mls_count integer not null default 0;
create index if not exists idx_agents_mls_count on agents (mls_count);

create or replace function fn_sync_mls_count()
returns trigger
language plpgsql
as $$
begin
  -- statement-level with transition tables; guarded branches keep the wrong-op
  -- transition-table reference from ever being executed
  if tg_op in ('INSERT', 'UPDATE') then
    update agents a set mls_count = (select count(*) from agent_mls m where m.agent_id = a.id)
     where a.id in (select agent_id from newtab);
  end if;
  if tg_op in ('DELETE', 'UPDATE') then
    update agents a set mls_count = (select count(*) from agent_mls m where m.agent_id = a.id)
     where a.id in (select agent_id from oldtab);
  end if;
  return null;
end;
$$;

drop trigger if exists trg_agent_mls_count_ins on agent_mls;
drop trigger if exists trg_agent_mls_count_upd on agent_mls;
drop trigger if exists trg_agent_mls_count_del on agent_mls;
create trigger trg_agent_mls_count_ins after insert on agent_mls referencing new table as newtab for each statement execute function fn_sync_mls_count();
create trigger trg_agent_mls_count_upd after update on agent_mls referencing old table as oldtab new table as newtab for each statement execute function fn_sync_mls_count();
create trigger trg_agent_mls_count_del after delete on agent_mls referencing old table as oldtab for each statement execute function fn_sync_mls_count();
