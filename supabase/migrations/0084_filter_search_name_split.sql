-- 0084: fn_filter_search uses the split page for top-bar searches (Phase B, part 2).
--
-- 0083 added fn_agent_page_ids, which resolves the relevance ordering as bounded per-group
-- queries instead of sorting 1.1M rows by a computed expression. This wires it in: when a
-- top-bar term is present the page of ids comes from that function and the rows are shaped by
-- joining the ids back with ordinality, so the order survives. Everything else -- the count,
-- the five per-row subqueries, every other mode and the MLS-scoped branch -- is untouched.
--
-- fn_agent_page_ids was verified against the original ORDER BY before this was wired up:
-- identical id lists for name terms at offsets 0/20/100, for the 3-group email form, and for
-- the no-term path.
...
begin
  ...
  -- Phase B: with a top-bar term, the relevance ordering is resolved by fn_agent_page_ids,
  -- which answers it as bounded per-group queries instead of sorting all 1.1M rows by a
  -- computed expression (6.7 s -> ~0.1 s). Only the page of ids changes; the row shaping
  -- below is the same, joined back with ordinality so the order survives.
