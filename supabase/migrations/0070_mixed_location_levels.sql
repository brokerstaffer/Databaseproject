-- 0070: location values may MIX geography levels — each include/exclude entry can be a
-- plain string (the filter's field) or {f, v} carrying its own level, so 'Miami, FL
-- (city) + Bucks, PA (county)' works in one filter. Levels OR together per side.
