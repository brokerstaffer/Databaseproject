-- 0092: the MLS-scoped branch no longer chooses a nested loop.
--
-- Client report: "applied a sales volume filter and was stuck on this screen for 20 seconds",
-- with Location + Sales volume + Office Search + MLS + Title active. Measured worse than
-- reported -- the count alone ran past 120 s.
--
-- See the comment in the function for the full diagnosis and the before/after table.
--
-- Also recorded here: the extended statistics created while diagnosing. They did not fix this
-- case (the bad estimates come from expression predicates, not cross-column correlation) but
-- they are correct and useful for ordinary city/state/brand filtering, so they stay.

CREATE STATISTICS IF NOT EXISTS stx_agents_office_geo (dependencies, ndistinct)
  ON office_city_key, office_state, office_county, office_zip FROM public.agents;
CREATE STATISTICS IF NOT EXISTS stx_agents_home_geo (dependencies, ndistinct)
  ON home_city_key, home_state, home_county, home_zip FROM public.agents;
CREATE STATISTICS IF NOT EXISTS stx_agents_transacted_geo (dependencies, ndistinct)
  ON most_transacted_city_key, transacted_state, most_transacted_county, most_transacted_zip FROM public.agents;
CREATE STATISTICS IF NOT EXISTS stx_agents_brand_geo (dependencies, ndistinct)
  ON brand, office_name, office_city_key, office_state FROM public.agents;
...
begin
  ...
  if scoped then
    ...
    --
    -- set_config(..., true) is SET LOCAL: it lasts to the end of the current transaction, which
    -- for an RPC call is this function call, so no other query is affected.
