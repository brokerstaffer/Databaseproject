-- 0014_orch_client_filter.sql
-- New filter key: p_filters->>'orchClientId'. The orchestrator (Masterinbox) records which
-- agents were built for which client in orch_client_leads (client_id, agent_id). Selecting a
-- client on the search screen narrows agents to exactly that client's leads — in office mode,
-- to the offices those agents belong to. Flows through search, CSV export and Clay/campaign
-- sends automatically (they all call this function with the same filters object).
