-- 0060 (C4): has-this-lead-ever-replied. bison_client_leads.replied is maintained by
-- the 6-hourly Bison sync (workspace-global replied sweep). Contact filter gains a
-- replied has/missing toggle; agent rows carry has_replied.
alter table bison_client_leads add column if not exists replied boolean not null default false;
...
declare v_where text; v_order text; v_sort_col text; v_dir text; v_count bigint; v_volume numeric; v_data jsonb; v_minoff int;
  v_gran text; v_lbl text; v_grp text; v_nn text;
  sel_mls uuid[]; scoped boolean := false; v_sc text; v_scord text;
begin
  if p_mode = 'location' then
    -- D4: location grain — group the (filtered) agents by their OFFICE location at the
    -- chosen granularity. Uses the stored city match-keys (0057) so city grouping is cheap.
    ...
    if v_gran = 'state' then
      v_lbl := 'upper(a.office_state)';
      v_grp := 'upper(a.office_state)';
      v_nn  := $c$a.office_state is not null and btrim(a.office_state) <> ''$c$;
    elsif v_gran = 'county' then
      v_lbl := $c$initcap(min(a.office_county)) || ', ' || upper(a.office_state)$c$;
      v_grp := 'lower(a.office_county), upper(a.office_state)';
      v_nn  := $c$a.office_county is not null and btrim(a.office_county) <> '' and a.office_state is not null$c$;
    else
      v_lbl := $c$min(a.office_city) || ', ' || upper(a.office_state)$c$;
      v_grp := 'a.office_city_key, upper(a.office_state)';
      v_nn  := $c$a.office_city_key is not null and a.office_state is not null$c$;
    end if;
