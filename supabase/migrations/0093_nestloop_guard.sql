-- 0093: keep the planner off nested loops whenever a grouped agent_mls_stats subquery is in play.
--
-- 0092 fixed this for the MLS-scoped branch after the client reported being "stuck for 20
-- seconds" (measured: past 120 s). It turns out the same pathology hits ordinary saved views,
-- which do not go through that branch at all -- so the guard belongs with the WHERE clause that
-- causes it, not with one branch.
--
-- The trigger is the SQL that fn_agent_where emits when an MLS filter meets a range filter:
--
--     id in (select agent_id from agent_mls_stats where mls_id = any(...)
--            group by agent_id having (sum(sales_volume) <= 10000000))
--
-- which happens on a plain MLS + Sales volume search, and inside any saved view that stores
-- both. The planner cannot estimate it, and the predicates around it are equally opaque -- the
-- location filter is an OR across three column pairs wrapped in coalesce/fn_city_embedded_state,
-- the title filter is fn_title_tokens(title) && array. Selectivities multiply as if independent,
-- the estimate collapses to a handful of rows, and it picks a nested loop -- then performs one
-- grouped aggregate lookup per outer row.
--
-- Measured on the "BHS Base Camp" saved view (6 locations + 1 MLS + a volume cap), server idle:
--
--                     nestloop on    off
--     count                4,284     126 ms
--     the 20-row page      3,151     129 ms
--
-- and on the reported filter set (Location + Sales volume + Office Search + MLS + Title):
--     count alone        >120,000     606 ms
--     through fn_filter_search        1,592 ms
--
-- Extended statistics were tried first and did not help: the bad estimates come from expression
-- predicates, not from cross-column correlation. (Those statistics are still in place from 0092
-- and remain useful for ordinary city/state/brand filtering.)
--
-- The guard is conditional -- it only fires when the emitted WHERE actually contains that
-- subquery, so every other search keeps the full range of plans. Checked against every scoped
-- combination before shipping and hash/merge was faster or within noise in all of them:
...
--
-- set_config(..., true) is SET LOCAL: it lasts to the end of the current transaction, which for
-- an RPC call is this one function call. Nothing outside it is affected.

CREATE OR REPLACE FUNCTION public.fn_nestloop_guard(p_where text)
 RETURNS void
 LANGUAGE plpgsql
 SET search_path TO 'public'
AS $function$
begin
  if p_where like '%agent_mls_stats%' then
...
begin
  ...
  if scoped then
    ...
    --     MLS + title     465 -> 462 ms      FULL SET    >60,000 -> 2,099 ms
    --
    -- (the guard itself now lives with the v_where assignments -- see fn_nestloop_guard)
