-- 0079: make the Title filter index-able.
--
-- The Title filter was CORRECT but slow — 3.9 s against 38 ms for an ordinary indexed filter
-- of similar selectivity (brand = 'Compass'), roughly 100x. It is in the main filter bar, so
-- operators hit it directly, and every combination that includes it inherited the cost.
--
-- Why it was slow: an agent's title can hold several roles in one string ("Salesperson, Team
-- Leader"), so matching splits the column and normalises each token, which makes "sales person"
-- match "Salesperson". Correct — but expressed inline it had to run per row:
--     Index Only Scan on idx_agents_title  (actual rows=119,520)
--       Rows Removed by Filter: 1,013,537
--       ->  Function Scan on unnest t   loops=1,133,057      <- a million times
--     Execution Time: 3,867 ms
-- No index could help: idx_agents_title holds the title AS WRITTEN ("Managing Broker") while
-- the predicate asks about a TRANSFORMED value ("managingbroker"). Same class of mismatch as
-- the city/county/state columns fixed in 0071-0073; this is the one that was missed.
--
-- Postgres will not index the expression directly — array(select ...) contains a subquery, and
-- index expressions may not. Hence the IMMUTABLE wrapper, which is what the GIN index is built
-- on and what fn_agent_where now emits.
--
-- Equivalence was verified against all 1,133,057 agents before anything was indexed: the
-- function and the old inline expression disagree on ZERO rows. Both forms return 119,520 for
-- 'Managing Broker'.
--
--     old form  3,867 ms   (per-row unnest, 1.13M loops)
--     new form    280 ms   (Bitmap Index Scan, Index Cond on fn_title_tokens)
--
-- The index is 1.4 MB — there are only a handful of distinct title tokens, so GIN stores very
-- few keys. Write cost is negligible at the scraper's ~1,000 rows per 45 minutes.

CREATE OR REPLACE FUNCTION public.fn_title_tokens(p text)
 RETURNS text[]
 LANGUAGE sql
 IMMUTABLE
 PARALLEL SAFE
AS $$
  select array(select regexp_replace(lower(t), '[^a-z0-9]', '', 'g')
                 from unnest(string_to_array(p, ',')) t)
$$;

-- Applied separately with CREATE INDEX CONCURRENTLY (cannot run inside a transaction, so this
-- statement is the record, not the thing that was executed):
--   CREATE INDEX CONCURRENTLY idx_agents_title_tokens
--     ON public.agents USING gin (public.fn_title_tokens(title));
